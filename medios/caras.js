...
async function carasScrap(cantidadMaxDeNoticias=20){
  const url = 'https://caras.perfil.com'
  let noticias = []
  ...
  try {
    ...
    $('article a').each((index, el) => {
      ...
      if (href && href.includes('/noticias/') && href.endsWith('.phtml')) {
    ...
    console.log(`[Caras] Enlaces de noticias detectados: ${newsLinks.length}`)
    ...
    for (let noticia of noticias) {
      try {
        ...
        const rawTitular = $n('h1.news__title').text().trim() || $n('h1.titular').text().trim() || $n('h1').first().text().trim()
        const rawResumen = $n('h2.news__headline').text().trim() || $n('h2.bajada').text().trim() || $n('h2').first().text().trim()
        ...
        console.error(`Error en noticia Caras: ${noticia.link}`, e.message)
      }
    }
